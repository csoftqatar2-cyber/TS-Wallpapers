...
| `CarTypeFileCheck.java` | Plain-JVM check of `CarTypeFile.familyOf` (the controller's `car.txt` key → Leopard / Denza / UNKNOWN mapping). Compile it together with `source/app/src/main/java/systems/sieber/fsclock/CarTypeFile.java` (command in its header); exits non-zero on a mismatch. |
