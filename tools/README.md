...
| `CarTypeFileCheck.java` | Plain-JVM check of `CarTypeFile.familyOf` (the controller's `car.txt` key → Leopard / Denza / UNKNOWN mapping). Compile it together with `source/app/src/main/java/systems/sieber/fsclock/CarTypeFile.java` with `android.jar` on the classpath (command in its header — the class logs through `android.util.Log`); exits non-zero on a mismatch. Also covers `familyOfWord` (the controller's `car_family.txt`). |
